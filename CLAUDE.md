...
**All six milestones (M0–M5) have landed.** Infra stack, FastAPI (`src/api/`), the SAM 3 worker (`src/segmentation_worker/`, CUDA), the React frontend (`src/web/`), and the prod deployment shape. **M3 (the SAM 3 worker image + real inference) is now VERIFIED end-to-end on an NVIDIA T4** (AWS `g4dn.xlarge`, via the dev-stack-on-GPU path in `docs/test-on-aws.md`): upload → SAM 3 segmentation → annotated image in the UI. Getting SAM 3 to build and run took real effort — the reproducible recipe and the non-obvious fixes (the **bf16 autocast** requirement, SAM 3's incomplete **dependency closure**, the **≥560 driver / CUDA 12.6** constraint, OpenCV-5-not-on-PyPI) are written up in **`docs/sam3-recipe.md`**. The one remaining unverified piece is the **M5 production cloud deploy** (`docker-compose.prod.yml` — Caddy/TLS + real-S3 in `docs/deploy.md`); the worker image it builds on is proven, but the prod overlay itself hasn't been run on a live public host.
...
Two hard-won SAM 3 specifics (see `docs/sam3-recipe.md` for the full story): (1) **inference must run under `torch.autocast("cuda", dtype=torch.bfloat16)`** — the model has hard-coded bf16 casts; this is entered *inside* `Segmenter.segment` because autocast is thread-local and we run in a `to_thread` worker thread, and `_to_numpy` casts bf16/fp16 outputs to fp32 since NumPy can't represent bf16. (2) SAM 3's `pip install -e .` is **not** a complete dependency set — its `__init__` eagerly imports training/data/video code, so the Dockerfile installs an extra closure (einops, pycocotools, psutil, …) derived by scanning the source's imports. OpenCV is pinned to **4.x** (`opencv-python-headless`) because OpenCV 5 has no PyPI wheel.
...
React SPA ──HTTP──> FastAPI ──> Postgres (Job rows)
                  ...
                  raw bytes        status/result updates
                       ▼              │
                    MinIO/S3   <─ NATS subjects ─> Python worker (SAM 3 + OpenCV, GPU)
```
...
- **M3 (#4) — real SAM 3 + OpenCV.** Replace the fake with real inference. Worker base image switches to `nvidia/cuda:12.6.x-cudnn-runtime` (PyTorch CUDA wheels, OpenCV — pinned to latest 4.x since v5 is not yet on PyPI, SAM 3 pinned from `facebookresearch/sam3`). **GPU host required; CPU unsupported.** HF checkpoint (`facebook/sam3.1`) is gated — needs `HF_TOKEN`; cache it in a named volume. Zero masks → `done` with `mask_count=0` and original image; inference error → `failed`. Model must be preloaded once, not per-request.
